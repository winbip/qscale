    ...
    public class UserDetails : ObjectBase
    {
        ...
        private static UserDetails CreateEntity(UserDetails userdetails)
        {
            ...
            try
            {
                ...
                if (SqlFunction.IsFoundStringTypeValue("select LogInName from tblUserDetails where LogInName='" + userdetails.pLogInName + "' and CompanyId=" + userdetails.pCompanyDetails.CompanyId + ""))
                {
                    throw new ExceptionWithControl("Error", "Login Name Exists", "This Login Name already in use. Try different Login Name.", "LogInName");
                }

                userdetails.pUserId = IdGenerator.CreateNewId("tblUserDetails", "UserId", userdetails.pConnectionString);

                SqlFunction.InsertData("INSERT INTO tblUserDetails(UserId, LogInName, LogInPassword, FullName, UserDesignation, CompanyId) VALUES(" + userdetails.pUserId + ",'" + userdetails.pLogInName + "','" + userdetails.pLogInPassword + "','" + userdetails.pFullName + "','" + userdetails.pUserDesignation + "',1)");
